...
- **導言段**：每首曲第一段不縮排，第一句設成金色：到第一個逗號或句號為止，括號裡的標點不算；段首是「人名（原文名，生卒年）」時到括號結束為止，例如「格蘭查尼（Marcel Grandjany，1891－1975）」。第七輪起不再用第一行（`::first-line` 會隨螢幕寬度切在詞中間）。超過約兩行（40 個全形字寬，拉丁字母算半個）還沒結束的句子就不上色。規則在 `src/domain/text-breaks.js` 的 `leadPhraseLength()`，只換顏色，不影響量測與分頁。第六輪起不再放大、不加左側金線。導言排不下開頭頁時，接到下一頁的部分是一般內文（`cutParagraph` 讓尾段不帶導言標記）。
...
電子書涵蓋整份印刷三折頁，頁序為：封面 → 節目單 → 樂曲解說 → 六位演出者 → 上海商銀封底。

- **節目單**：原本的目錄升級為印刷版的節目單。每個編制區塊只在開頭列出編制與演出者（中場休息之後重新列出），接著是中英文曲名；樂章與豎琴獨奏的各首曲目各自是一個原子（`contents-movement`、`contents-work`），所以頁面斷在行與行之間，不會為了整塊而留下半頁空白。整份節目單仍可點選跳到該首解說。
- **演出者**：`person-banner` 是 `PAGE_OPENERS` 之一，每位演出者從新頁開始：固定高度的照片帶（`--page-height * 0.3`）、樂器、姓名，然後是印刷品上的介紹。照片帶高度固定且 `<img>` 帶著自己的尺寸，所以量測在圖片載入前後一致。
- **封底**：`back-cover` 是整頁原子，整幅廣告以 `object-fit: contain` 置中，四周退到頁面邊緣。
- **版權頁取消**：主辦與贊助改列在封面，精簡模式也保留。
- **字型**：節目冊只用內嵌的獅尾四季春加糖與 Bodoni Moda；Noto Sans TC 與 Noto Serif TC 改由 `src/lib/cjk-webfonts.js` 在其他畫面出現時才載入，替書本頁省下約 1.8MB。
