...
5. **Programme list** — numbered scenes or musical works with instrumentation and duration.
...
- Images are optional at chapter level.
- The page must preserve hierarchy and spacing when no image exists.
- Client-supplied photography should retain its documentary character; do not smooth skin, invent people, or force every image into one crop.
- Portraits, rehearsal images, posters, and sponsor marks must carry descriptive alt text or be marked decorative when appropriate.
- Original PDFs remain private files served through short-lived signed URLs.
