import backCoverUrl from "../../assets/back-cover-shanghai-bank.webp?url";
import coverUrl from "../../assets/cover-moonlight-promise.webp?url";
import chanChingChing from "../../assets/people/chan-ching-ching.webp?url";
import chenWenWan from "../../assets/people/chen-wen-wan.webp?url";
import huangYunYu from "../../assets/people/huang-yun-yu.webp?url";
import liChi from "../../assets/people/li-chi.webp?url";
import saiAiLing from "../../assets/people/sai-ai-ling.webp?url";
import yangJuiSe from "../../assets/people/yang-jui-se.webp?url";

/**
 * The performers, in the order the printed tri-fold introduces them: the harp
 * soloist first, then the strings, then the piano. Photographs and text come
 * from the printed sheet; each portrait carries its own size so a page can be
 * measured before the picture has loaded.
 */
export const moonlightPromisePeople = [
  {
    slug: "sai-ai-ling",
    role: "豎琴",
    name: "彩愛玲",
    name_en: "Sai Ai Ling",
    portrait: { url: saiAiLing, width: 640, height: 479 },
    paragraphs: [
      "榮獲2025年首屆日本‒臺灣友好藝術家獎（第一回日本臺灣友好藝術家賞）。",
      "彩愛玲出生於東京，自幼展現音樂天賦，畢業於國立音樂大學，並於東京藝術大學（東京藝大）別科修畢，並師從豎琴名家篠崎綾子。自幼受為音樂家的祖父母影響，開始學習古典音樂。作為豎琴獨奏家，她曾於日本、美國、法國、瑞典、韓國、臺灣等地演出。二十餘年來，她不僅活躍於音樂會舞臺，也跨足朗讀會、落語、時裝秀等多元藝術形式，展現跨界風貌。",
      "在古典音樂領域，她曾與世界知名長笛演奏家工藤重典、薩克斯風演奏家須川展也等人合作演出。除古典領域外，她亦參與流行音樂的錄音及現場演出，合作對象包括Van Dyke Parks、細野晴臣等國際知名音樂人。",
      "2004年，她參與美國麻省理工學院出版之《Computer Music Journal》的演出。2007年發行首張專輯《花一輪》（King Records）正式出道。除古典音樂外，她亦活躍於跨界領域，與流行音樂藝人合作超過二十年，參與錄音及現場演出，其作品曾被選入臺灣紀錄片及NHK BS電視節目。",
      "彩愛玲同時也是作曲家，其原創作品曾為多部臺灣紀錄片譜寫主題音樂，如《老兵挽歌》、《吳さんの包丁》與《心の故郷》（導演：林正幸/Creative 21）。",
      "自2022年起，她開始教授即興豎琴音樂冥想課程，並以「ZENHARP」為主題，結合東方思想中的陰陽五行，開發全新即興教學與演奏方式，並發行專輯《Water Meditation》。為推廣豎琴作為療癒樂器，她除演出外，亦積極於日本與臺灣舉辦音樂冥想講座與示範活動。彩愛玲活躍於多元音樂風格，持續以獨特音色與創意打動國際樂壇。",
    ],
  },
  {
    slug: "li-chi",
    role: "小提琴",
    name: "李季",
    name_en: null,
    portrait: { url: liChi, width: 504, height: 632 },
    paragraphs: [
      "曾任國立臺南藝術大學音樂學系專任副教授。長榮交響樂團首席、國家交響樂團特約首席與法國阿爾薩斯省Strasbourg音樂院之小提琴教授。",
      "1982年獲法國政府公費獎學金赴法深造，取得巴黎師範音樂院室內樂演奏家、小提琴演奏家高等文憑。旅法期間，任職法國東部洛林省Nancy市交響樂團首席18年。除樂團演出外，演出足跡遍及歐、美、日本、臺灣、中美洲及非洲，並灌製多張CD，屢獲Le monde de la musique、Diapason等雜誌好評。演出曲目自巴洛克時期作品到現代樂。",
      "2005年返臺定居，任教於國立臺南藝術大學直到退休。經常受邀於不同的國內外音樂營，擔任小提琴、室內樂指導及樂團指揮。",
    ],
  },
  {
    slug: "chan-ching-ching",
    role: "小提琴",
    name: "詹青青",
    name_en: null,
    portrait: { url: chanChingChing, width: 640, height: 426 },
    paragraphs: [
      "小提琴家詹青青活躍於室內樂、管弦樂及跨領域藝術演出。",
      "2015年起持續參與〈緣　室內樂集〉鋼琴五重奏系列，2017年起積極探索多元室內樂編制，策劃二重奏、弦樂三重奏及雋永弦樂四重奏演出。",
      "2020年發展《畫灑樂章》跨界系列，結合小提琴與畫家現場創作。亦曾隨台北愛樂室內及管弦樂團國際巡演。2026年與鋼琴家陳致宜博士推出《純粹和諧》，探索鋼琴與多把小提琴的多重聲響。",
      "畢業於美國新英格蘭音樂學院，並取得哈特音樂學院演奏文憑，師承James Buswell IV、胡乃元及Katie Lansdale、蘇正途、高安香等教授。",
    ],
  },
  {
    slug: "yang-jui-se",
    role: "中提琴",
    name: "楊瑞瑟",
    name_en: null,
    portrait: { url: yangJuiSe, width: 426, height: 640 },
    paragraphs: [
      "國立臺灣師範大學音樂系畢業後，獲得教育部公費留學獎學金負笈法國，並在巴黎師範音樂院榮獲高級演奏家文憑畢業。",
      "2015年起，已發行數張錄音專輯：《采歌》、《弦中之歌》、《對話6×6》、《楊瑞瑟：臺灣印象──音．畫》、《心聆I,II, III》等，專輯內容從中提琴的經典作品，乃至跨樂種與風格的創意性突破，兼具傳統與當代。2023年發行之《楊瑞瑟：臺灣印象──音．畫》榮獲第34屆金曲獎「最佳藝術音樂專輯」，並入圍最佳演奏獎項。",
      "現任國立臺灣師範大學音樂系教授，春詠藝術文化基金會董事暨藝術總監，臺灣中提琴室內樂團團長。",
    ],
  },
  {
    slug: "huang-yun-yu",
    role: "大提琴",
    name: "黃韻宇",
    name_en: null,
    portrait: { url: huangYunYu, width: 445, height: 468 },
    paragraphs: [
      "畢業於國立藝專音樂科，獲美國北德州大學大提琴演奏碩士，並赴加拿大卑詩大學博士班深造，現任教於文化大學音樂系。",
      "返臺後曾於國家演奏廳舉辦獨奏會，並活躍於室內樂舞台，參與名家鋼琴三重奏、台北大提琴重奏團及The Singhini Consort辛吉尼樂團等。2010年創立〈緣　室內樂集〉，巡演各地文化中心及國家演奏廳，持續策劃「理性與感性」鋼琴五重奏系列；2016年組成〈雋詠弦樂四重奏〉。",
      "多年致力鈴木教學法於大提琴教育之實踐，曾赴義大利、美國、日本參與國際鈴木教學法大會，並策劃多場大提琴重奏音樂會。特別是2019年於中山堂典雅的光復廳舉行《水晶燈下的五十把大提琴‒琴音悠揚》。2026年翻譯Tanya Carey《拉大提琴很輕鬆》中文版出版，持續深耕演奏、教學與音樂推廣。",
    ],
  },
  {
    slug: "chen-wen-wan",
    role: "鋼琴",
    name: "陳文婉",
    name_en: null,
    portrait: { url: chenWenWan, width: 428, height: 640 },
    paragraphs: [
      "任職中原大學風雅頌音樂中心主任、通識中心專任教授、以及實踐大學音樂系鋼琴教授、臺灣室內樂推廣協會理事。",
      "畢業於臺北藝術大學音樂系與美國舊金山音樂院。畢業後隨即留在舊金山音樂院任職伴奏職務。",
      "返國後，教學之餘經常以室內樂和鋼琴合作型態活躍於國內樂壇，多年來，致力於開拓室內樂的曲目。於2010年成立〈緣　室內樂集〉。除了每年定期策劃〈緣　室內樂集〉鋼琴五重奏「理性與感性」系列的演出活動外，更致力推廣音樂進入社區，提倡美育，為藝術文化耕耘。近年與鋼琴家陳冠宇組鋼琴二重奏組合，呈現室內樂不同的面貌。",
    ],
  },
];

/**
 * The book opens on the printed cover panel: lockup, date and credits as
 * printed. Its sides carry a mirrored reach of its own artwork, so the page can
 * be filled edge to edge without cropping anything the printer set.
 */
export const moonlightPromiseCoverArtwork = {
  url: coverUrl,
  width: 1420,
  height: 2028,
  alt: "月光下的約定：2026 年 9 月 25 日（五）19:30，國家兩廳院演奏廳",
};

/** The sponsor's page closes the book, exactly as it closes the printed sheet. */
export const moonlightPromiseBackCover = {
  url: backCoverUrl,
  width: 947,
  height: 2028,
  alt: "上海商業儲蓄銀行：科技領航、顧客為本、連繫國際",
};
